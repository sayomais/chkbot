...
from all.functions.func_auths.func_xero import *

nombregate = "Xero"
tipogate = "Stripe Auth"
cargogate = "(Auth)"
cmdgate = "xr"


@Techie(f'{cmdgate}')
async def xerocmd(_, message):
    tiempo = time.time()
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('xr', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'xr', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('xr', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
