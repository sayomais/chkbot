...
from all.functions.func_avs.func_hinata import *
...
async def ausexcmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('hn', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'hn', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('hn', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
