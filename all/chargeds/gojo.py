...
from all.functions.func_charged.func_gojo import auth
...
async def gojocmd(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('go', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'go', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('go', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
