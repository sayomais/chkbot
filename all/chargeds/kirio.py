...
from all.functions.func_charged.func_kirio import *
...
collection.update_many({}, {"$set": {"tareas_en_proceso": 0}})
collection.update_many({}, {"$set": {"antispam": 60, }})
...
async def yamadacmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('pp', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'pp', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('pp', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
...
━━━━━━━━「ELAINA」━━━━━━━━</b>"""
    ...
    code = await misakifunc(cc,mes,ano,cvv)
    if code == True:
        mensaje = 'Charged 0.01'
        status = 'Approved!'
        logo = '🟩'
    elif code == False:
        mensaje = 'Charged 0.01'
        status = 'Approved!'
        logo = '🟩'
    elif code == "INVALID_BILLING_ADDRESS":
        mensaje = 'INVALID_BILLING_ADDRESS'
        status = 'Approved!'
        logo = '🟩'
    elif code == "INVALID_SECURITY_CODE":
        mensaje = 'INVALID_SECURITY_CODE'
        status = 'Approved!'
        logo = '🟩'
    elif code == "EXISTING_ACCOUNT_RESTRICTED":
        mensaje = 'EXISTING_ACCOUNT_RESTRICTED'
        status = 'Approved!'
        logo = '🟩'
    elif code == "RISK_DISALLOWED":
        mensaje = 'NFS / RISK_DISALLOWED'
        status = 'Approved!'
        logo = '🟩'
    else:
        mensaje = code
