...
from all.functions.func_charged.func_rimuru import *
...
@Techie(f'{cmdgate}')
async def rimuruicmds(_, message):
    tiempo = time.time()
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('rm', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'rm', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('rm', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
...
━━━━━━━━「ELAINA」━━━━━━━━</b>"""
    ...
    texto_1 = carga1.format(cc=cc, mes=mes, ano=ano, cvv=cvv, brand=x.get("vendor"), level=x.get("level"), typea=x.get("type"),
                            country_flag=x.get("flag"), nombregate=nombregate, ID=ID, FIRST=FIRST, tipogate=tipogate, user_plan=usuario["plan"])

    ñ = await message.reply(text=texto_1, quote=True)

    try:
        resultado = await rimurufunc(cc, mes, ano, cvv)
        if resultado == "success":
            mensaje = "Thanks For Your Donate"
            status = 'Approved!'
            logo = '🟩'
        elif resultado == "Transaction Failed. 202-Insufficient Funds":
            mensaje = 'Transaction Failed. 202-Insufficient Funds!'
            status = 'Approved!'
            logo = '🟩'
        elif resultado == 'Transaction Failed. 225-Decline CVV2/CID Fail':
            mensaje = 'Transaction Failed. 225-Decline CVV2/CID Fail!'
            status = 'Approved!'
            logo = '🟩'
        else:
            mensaje = resultado
...
<i>Gateways</i> ➣ <i>{nombregate}|{tipogate}</i>
<i>Checked by</i> ➣ <a href="tg://user?id={ID}">{FIRST}</a> [<code>{usuario["plan"]}</code>]
━━━━━━━━「ELAINA」━━━━━━━━</b>"""
