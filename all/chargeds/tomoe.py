...
from all.functions.func_charged.func_tomoe import tomorifun
...
async def minato(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('to', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'to', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('to', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
...
━━━━━━━━「ELAINA」━━━━━━━━</b>"""
    ...
    try:
        resultado = await tomorifun(cc, mes, ano, cvv)
        if resultado == "This transaction cannot be processed. Please enter a valid Credit Card Verification Number.":
            mensaje = 'This transaction cannot be processed. Please enter a valid Credit Card Verification Number.'
            status = 'Approved!'
            logo = '🟩'
        else:
            mensaje = resultado
