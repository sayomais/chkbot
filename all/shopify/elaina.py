...
from all.functions.func_shopi.func_elaina import auto_sho_async
...
async def sylphiettecmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('ela', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'ela', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('ela', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
