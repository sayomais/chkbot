...
from all.functions.func_shopi.func_emma import auto_sho_async
...
async def arakicmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('ea', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'ea', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('ea', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
