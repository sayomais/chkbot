...
from all.functions.func_shopi.func_iruma import auto_sho_async
...
async def yamadacmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('ia', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'ia', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('ia', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
