...
from all.functions.func_shopi.func_izumi import auto_sho_async
...
async def izumicmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('iz', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'iz', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('iz', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
