...
from all.functions.func_shopi.func_mikasa import auto_sho_async

nombregate = "Mikasa"
tipogate = "Sh + CyberSource"
cargogate = "($29.37)"
cmdgate = "as"


@Techie(f'{cmdgate}')
async def itsukacmds(_, message):
    tiempo = time.time()
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('as', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'as', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('as', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
...
━━━━━━━━「ELAINA」━━━━━━━━</b>"""
    ...
    try:
        ...
        if resultado == "Charged":
            mensaje = "Charged $29.37 USD!"
            status = 'Approved!'
            logo = '🟩'
        elif resultado == "Insufficient funds in the account":
            mensaje = "Insufficient funds in the account"
            status = 'Approved!'
            logo = '🟩'
        elif resultado == "Invalid card verification number":
            mensaje = "Invalid card verification number"
            status = 'Approved!'
            logo = '🟩'
        else:
            mensaje = resultado
