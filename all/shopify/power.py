...
from all.functions.func_shopi.func_power import auto_sho_async
...
async def itsukacmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('pw', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'pw', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('pw', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
