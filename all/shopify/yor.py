...
from all.functions.func_shopi.func_yor import auto_sho_async
...
async def arakicmds(_, message):
    ...
    if usuario is not None and usuario.get("role") == "Owner":
        ...
        if not comandos_habilitados.get('yor', {}).get('enabled', False):
            reason = comandos_habilitados.get(
                'yor', {}).get('reason', "Unknown")
            date = comandos_habilitados.get('yor', {}).get('date', "Unknown")
            _textoff = f"""<b>━━━━━━━━「ELAINA」━━━━━━━━
...
━━━━━━━━「ELAINA」━━━━━━━━</b>"""
    ...
    try:
        ...
        if resultado == 'Charged':
            mensaje = 'Charged $19.55 USD'
            status = 'Approved!'
            logo = '🟩'
        elif resultado == 'CVC Declined | N7 : Decline for CVV2 failure':
            mensaje = 'CVC Declined | N7 : Decline for CVV2 failure'
            status = 'Approved!'
            logo = '🟩'
        else:
            mensaje = resultado
